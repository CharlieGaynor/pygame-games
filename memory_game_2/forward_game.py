...
def run_forward_game(starting_number=0):
    ...
    while True:
            ...
            if not draw_msg:
                ...
                if numbers_shown <= N+1:
                    ...
                    end_button = button(x=SURFACE_SIZE[0]/2,
                            y=SURFACE_SIZE[1]/2,width=1,height=1,text='What did you see?')
            ...
            if confirmed_input is not None:
                ...
                if confirmed_input == sequence[:-1]:
                    ...
                    if needs_logging:
                        ...
                        df = df.append({'Date' : d1, 'Sequence length': N , 'is_backwards': 0,'is_completed' : 1,'streak_so_far' : N,'Time': now},ignore_index = True)
                        needs_logging = False
                    ...
                    wrong = True
                    feedback_button = button(x=SURFACE_SIZE[0]/2,
                        y=SURFACE_SIZE[1]/2-125,width=1,height=1,text='That\'s wrong! You put:')
                    ...
                    true_sequence1 = button(x=SURFACE_SIZE[0]/2,
                        y=SURFACE_SIZE[1]/2 + 35,width=1,height=1,text='The sequence was:')
                    true_sequence2 = button(x=SURFACE_SIZE[0]/2,
                        y=SURFACE_SIZE[1]/2+125,width=1,height=1,text=f'{"".join(sequence[:-1])}')
                    ...
                    if needs_logging:
                        ...
                        if first_time == 1:
                            df = df.append({'Date' : d1, 'Sequence length': N , 'is_backwards': 0,'is_completed' : 0,'streak_so_far' : 0,'Time': now},ignore_index = True)
                            needs_logging = False
                        else:
                             df = df.append({'Date' : d1, 'Sequence length': N , 'is_backwards': 0,'is_completed' : 0,'streak_so_far' : N-1,'Time': now},ignore_index = True)
                             needs_logging = False
        ...
        if show_feedback:
            ...
            if wrong:
                ...
                true_sequence1.draw_text(screen)
                true_sequence2.draw_text(screen)

        if draw_msg:
            msg_button.draw_text(screen,size=80)

        if draw_end_button:
            end_button.draw_text(screen, size= 65)
